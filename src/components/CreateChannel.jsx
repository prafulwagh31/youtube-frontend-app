...
const CreateChannel = () => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    let channelData = { ...formData, owner: user?.id };
    try {
      let result = await axios.post(
        "https://youtube-backend-pjh2.onrender.com/youtube/createChannel",
        channelData,
        {
          headers: {
  ...
  const fetchCurrentUser = async () => {
    try {
      let { data } = await axios.get(
        `https://youtube-backend-pjh2.onrender.com/youtube/getSingleUser/${user?.id}`
      );
      console.log(data);
      if (data) {
